...
void ClapTrap::takeDamage(unsigned int amount) {
	if (amount <= 0){
		std::cout << "ClapTrap can't take negative or zero damage!" << std::endl;
		return;
	}
	std::cout << "ClapTrap " << _name << " takes " << amount << " points of damage!" << std::endl;
	_hitPoints -=(int)amount;
	if (_hitPoints <= 0)
		std::cout << "ClapTrap " << _name << " is dead after attack!" << std::endl;

}

void ClapTrap::beRepaired(unsigned int amount) {
	if (amount <= 0){
		std::cout << "ClapTrap can't be repaired for negative or zero points!" << std::endl;
		return;
	}
	if (_hitPoints < 1){
		std::cout << "ClapTrap " << _name << " can't be repaired because it's already dead!" << std::endl;
		return;
	}
	if (_energyPoints < 1){
	...
	_energyPoints--;
	_hitPoints += (int)amount;
	std::cout << "ClapTrap " << _name << " was repaired for " << amount << " points!" << std::endl;
}
